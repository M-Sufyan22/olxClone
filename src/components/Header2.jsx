import React from "react";
import "../css/header2.css";


function Header2() {
    function showMenu(){
        var ham = document.getElementById('ham');
        ham.classList.toggle('hide');
        var cc = document.getElementById('cc');
        cc.classList.toggle('hide');
        
       };
    return(
        
        <div className="header_container2">
            <div className="header2_sec1">
                <div onClick={showMenu} className="menu_icon">
                    <svg id="ham" width="24px" height="24px" viewBox="0 0 1024 1024" fill="#002f34"><path d="M896 682.667l42.667 42.667-42.667 42.667h-768l-42.667-42.667 42.667-42.667h768zM896 469.333l42.667 42.667-42.667 42.667h-768l-42.667-42.667 42.667-42.667h768zM896 256l42.667 42.667-42.667 42.667h-768l-42.667-42.667 42.667-42.667h768z"></path></svg>
                    <svg id="cc"  width="24px" height="24px" viewBox="0 0 1024 1024" fill="#002f34" className="hide"><path d="M 878.336 85.333 l -366.336 366.315 l -366.336 -366.315 h -60.331 v 60.331 l 366.336 366.336 l -366.336 366.336 v 60.331 h 60.331 l 366.336 -366.336 l 366.336 366.336 h 60.331 v -60.331 l -366.315 -366.336 l 366.315 -366.336 v -60.331 Z"></path></svg>
                </div>
                <div className="site_logo2">
                    <svg width="38px" height="38px" viewBox="0 0 1024 1024">
                    <path fill="#002f34" d="M661.333 256v512h-128v-512h128zM277.333 298.667c117.824 0 213.333 95.531 213.333 213.333s-95.509 213.333-213.333 213.333c-117.824 0-213.333-95.531-213.333-213.333s95.509-213.333 213.333-213.333zM794.496 384l37.504 37.504 37.504-37.504h90.496v90.496l-37.504 37.504 37.504 37.504v90.496h-90.496l-37.504-37.504-37.504 37.504h-90.496v-90.496l37.504-37.504-37.504-37.504v-90.496h90.496zM277.333 426.667c-47.061 0-85.333 38.293-85.333 85.333s38.272 85.333 85.333 85.333c47.061 0 85.333-38.293 85.333-85.333s-38.272-85.333-85.333-85.333z"></path>
                    </svg>
                </div>
                <div className="header2_city_sel">
                    <div className="header2_selected_city">
                        <span>Pakistan</span>
                        <svg width="18px" height="18px" viewBox="0 0 1024 1024" data-aut-id="icon" fill="#002f34"><path  d="M512 85.333c211.755 0 384 172.267 384 384 0 200.576-214.805 392.341-312.661 469.333v0h-142.656c-97.856-76.992-312.683-268.757-312.683-469.333 0-211.733 172.267-384 384-384zM512 170.667c-164.672 0-298.667 133.973-298.667 298.667 0 160.021 196.885 340.523 298.453 416.597 74.816-56.725 298.88-241.323 298.88-416.597 0-164.693-133.973-298.667-298.667-298.667zM512.006 298.66c94.101 0 170.667 76.565 170.667 170.667s-76.565 170.667-170.667 170.667c-94.101 0-170.667-76.565-170.667-170.667s76.565-170.667 170.667-170.667zM512.006 383.994c-47.061 0-85.333 38.272-85.333 85.333s38.272 85.333 85.333 85.333c47.061 0 85.333-38.272 85.333-85.333s-38.272-85.333-85.333-85.333z"></path></svg>
                    </div>               
                </div>
            </div>
              
        <div className="header_sec_2">
            <div className="searchbox2">   
                <span>
                    <svg width="20px" height="20px" fill="#002f34" viewBox="0 0 1024 1024">
                    <path  d="M448 725.333c-152.917 0-277.333-124.416-277.333-277.333s124.416-277.333 277.333-277.333c152.917 0 277.333 124.416 277.333 277.333s-124.416 277.333-277.333 277.333v0zM884.437 824.107v0.021l-151.915-151.936c48.768-61.781 78.144-139.541 78.144-224.192 0-199.979-162.688-362.667-362.667-362.667s-362.667 162.688-362.667 362.667c0 199.979 162.688 362.667 362.667 362.667 84.629 0 162.411-29.376 224.171-78.144l206.144 206.144h60.352v-60.331l-54.229-54.229z"></path>
                    </svg>
                </span>
                <input readOnly type="text" name=""placeholder="Find Cars, Mobile Phones and more..."/>
            </div> 
        </div>
    
        <button className="header2-sell-btn">
            <span>+ </span>
                SEll
        </button>
        </div>
    );
}

export default Header2;